...
def create_environment(env_id, render_mode='human'):
    ...
    if 'ALE/' in env_id or 'Atari' in env_id:
        # Atari 환경인 경우
        try:
            # 최신 gymnasium에서는 FrameStack이 다른 경로에 있을 수 있음
            try:
                ...
                try:
                    ...
                    # 백업: 직접 FrameStack 구현 또는 다른 경로 시도
                    import gymnasium.wrappers as wrappers
                    AtariPreprocessing = wrappers.AtariPreprocessing
                    FrameStack = getattr(wrappers, 'FrameStack', None) or getattr(wrappers, 'FrameStackObservation', None)
                    
            if FrameStack is None:
                raise ImportError("FrameStack wrapper not found")
